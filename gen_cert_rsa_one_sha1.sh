...
openssl req -new -sha1 -key $pr_dir/$leaf_name.pkcs8.key -config $leaf_name.cnf -out $crt_dir/$leaf_name.csr

openssl ca -config $ca_name.cnf -md sha1 -in $crt_dir/$leaf_name.csr -out $crt_dir/$leaf_name.crt
