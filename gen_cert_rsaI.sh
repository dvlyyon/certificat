...
openssl ca -config $ca1_name.cnf -in $crt_dir/$leaf_name.csr -out $crt_dir/$leaf_name.crt


openssl pkcs12 -export -name "My Certificates $leaf_name" -out $leaf_name.pfx -inkey private/$leaf_name.key -in $leaf_name.crt 
openssl pkcs12 -export -name "My Certificates $leaf_name" -out $leaf_name.c0.pfx -inkey private/$leaf_name.key -in $leaf_name.crt -certfile $ca1_name.crt 
...
cat $crt_dir/$leaf_name.crt $crt_dir/$ca1_name.crt | sed -n '/-BEGIN CERT/,/-END CERT/p' > $crt_dir/$leaf_name.chain.crt
