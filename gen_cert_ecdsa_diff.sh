...
leaf_name4="ecdsa_myne1_nohost"
leaf_name5="ecdsa_myne1_ipv6"
...
openssl ecparam -name prime256v1 -genkey -out $pr_dir/$leaf_name4.key
...
openssl ecparam -name prime256v1 -genkey -out $pr_dir/$leaf_name5.key
openssl pkcs8 -topk8 -nocrypt -in $pr_dir/$leaf_name5.key -out $pr_dir/$leaf_name5.pkcs8.key
openssl req -new -sha512 -key $pr_dir/$leaf_name5.pkcs8.key -config $leaf_name5.cnf -out $crt_dir/$leaf_name5.csr
...
openssl ca -config $ca_name.cnf -in $crt_dir/$leaf_name5.csr -out $crt_dir/$leaf_name5.crt
...
openssl pkcs12 -export -name "My Certificates $leaf_name5" -out $leaf_name5.pfx -inkey $pr_dir/$leaf_name5.key -in $leaf_name5.crt 
